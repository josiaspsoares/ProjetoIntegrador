...
void cadastroPartidasManualmente(int tamanho, PLAY *Partida, DATA *Fase){
    ...
    for(i = 0, j = 0; i < tamanho; i++, j += 2){
        system("cls");
        printf(C_GREEN "    *** InformaÁıes das Partidas *** \n\n" NONE);
        printf("\t     --- JOGO %d --- \n",i+1);
    ...
    for(i = 0, j = 0; i < 16; i += 2, numJogo++){
        ...
        if((Oitavas[i].gols) >  (Oitavas[i + 1].gols)){
        	...
        	do{ 
               system("cls"); 
                system("color 0F");
                printf(C_MAGENTA"\t  *** DISPUTA POR P NALTES ***\n" NONE);
                printf("\n\t\t--- JOGO %d --- \n", numJogo );
...
void exibirPodio(){
    ...
    printf(C_BLUE "\t<<< P”DIO DA COPA >>>\n\n" NONE);
    printf(C_GREEN " -> SELE«√O CAMPE√: %s\n" NONE, Podio[0].equipes);
    printf(C_MAGENTA " -> SELE«√O VICE-CAMPE√: %s\n" NONE, Podio[1].equipes);
    printf(C_RED " -> SELE«√O TERCEIRA-COLOCADA: %s\n" NONE, Podio[2].equipes);
